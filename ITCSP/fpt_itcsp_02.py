...
def print_dictionary(d: dict, num) -> None:
    ...
    for index, words in (text):
        print(f"{words}:({frequency})")

    dictionary = {f"{words}:{num}"}
...
def get_sorted_dictionary(d: dict) -> dict:
    '''
    Function returns sorted version of the given dictionary
    ...
    '''
    sorted_dictionary = words.sorted()
